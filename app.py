...
REPLICATE_GENERATE_MODEL = os.environ.get(
    "REPLICATE_GENERATE_MODEL",
    "black-forest-labs/flux-schnell"
).strip()
REPLICATE_EDIT_MODEL = os.environ.get("REPLICATE_EDIT_MODEL", "").strip()

REPLICATE_GENERATE_PROMPT_KEY = os.environ.get(
    "REPLICATE_GENERATE_PROMPT_KEY",
    "prompt"
).strip()
REPLICATE_EDIT_IMAGE1_KEY = os.environ.get(
    "REPLICATE_EDIT_IMAGE1_KEY",
    "image"
).strip()
REPLICATE_EDIT_IMAGE2_KEY = os.environ.get(
    "REPLICATE_EDIT_IMAGE2_KEY",
    "image2"
).strip()
REPLICATE_EDIT_PROMPT_KEY = os.environ.get(
    "REPLICATE_EDIT_PROMPT_KEY",
    "prompt"
).strip()
...
def run_replicate_generate(prompt: str) -> bytes:
    ...
    if not REPLICATE_API_TOKEN or not replicate_client:
    ...
    output = replicate_client.run(REPLICATE_GENERATE_MODEL, input=model_input)
...
def run_replicate_edit(image1_bytes: bytes, image1_name: str, image2_bytes: bytes, image2_name: str, prompt: str) -> bytes:
    ...
    output = replicate_client.run(REPLICATE_EDIT_MODEL, input=model_input)
    return read_replicate_output(output)
